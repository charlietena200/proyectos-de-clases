...
int opcion1();
int Lagrange(int n);
int FormulaNewton(int n, float *x, float *y, float xi);
int spline(int n);
	...
	do{
		...
		switch(op){
			...
			case 2:
				...
				float x[n];
				float xi, yi;
				printf("\nIngresa valores la la columna xi");
				for(i=0;i<n;i++){
					printf("\n valor %d=",i);
					scanf("%f", &x[i]);
				}
				printf("\nIngresa valores la la columna f(xi)");
				for(i=0;i<n;i++){
					printf("\n valor f(%d)=",i);
					scanf("%f", &y[i]);
				}
    			...
    			yi=FormulaNewton(n,x,y,xi);
    			printf("Valor interpolado=%.6f",yi);
				break;
